...
using namespace std;
int n,m;
int bessie[101][101],elsie[101][101];
bool dpB[101][10001],dpE[101][10001];
int main(){
    int a,b,c,d;
    cin >> n >> m;
    for(int i=0;i<m;i++){
    ...
    for(int i=1;i<=n;i++){
        for(int j=0;j<=10000;j++){
            if(dpB[i][j]==true){
                for(int k=1;k<=n;k++){
                    if(bessie[i][k]!=0&&j+bessie[i][k]<=10000){
                        dpB[k][j+bessie[i][k]]=true;
                    }
                }
            }
            if(dpE[i][j]==true){
                for(int k=1;k<=n;k++){
                    if(elsie[i][k]!=0&&j+elsie[i][k]<=10000){
                        dpE[k][j+elsie[i][k]]=true;
                    }
                }
            }
        }
